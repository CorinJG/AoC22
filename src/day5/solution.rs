...
fn move_crates_9000(stacks: &mut Vec<Vec<char>>, num: u32, src: usize, dest: usize) {
    for _ in 0..num {
...
fn move_crates_9001(stacks: &mut Vec<Vec<char>>, num: u32, src: usize, dest: usize) {
    let mut popped_crates: Vec<char> = Vec::new();
    for _ in 0..num {
        popped_crates.push(
            stacks[src-1].pop().expect("instruction invalid - stack has insufficient crates")
        );
    }
...
fn main() {
    ...
    for instruction in INPUT.trim().lines() {
        ...
        if let [_, num, _, src, _, dest] = instruction_vec.as_slice() {
